...
public class NameParser {
  ...
  public static void main(String[] args) {
    ...
    for (String name : args) {
      ...
      System.out.println("NORM : " + normalize(name));
   ...
   * It will ignore a conceptual sec reference, remarks or notes on the nomenclatural status.
   * Please use parseExtended(String) if you need these.
   * In some cases the authorship parsing proves impossible and this parser will return null.
   * If you do not need the authorship please use parseIgnoreAuthors(String) instead which is more robust.
   * - canonical name
   * - final authorship
   * - sec/auct "concept" references
   * - nomenclatural status remarks
   * - other informal remarks
   */
  public <T> ParsedName<T> parse(final String scientificName) throws UnparsableException {
    ...
    ParsedName<T> pn = new ParsedName<T>();
    ...
    if (m.find()) {
      pn.nomStatus = StringUtils.trimToNull(m.group(1));
      name = m.replaceFirst("");
    }
    ...
    if (m.find()) {
      pn.sensu = StringUtils.trimToNull(m.group(1));
      name = m.replaceFirst("");
    }
    // extract other remarks
    m = EXTRACT_REMARKS.matcher(name);
    if (m.find()) {
      pn.remarks = StringUtils.trimToNull(m.group(1));
      name = m.replaceFirst("");
    }
    ...
    if (!parsed) {
      ...
      if (!parsed) {
        ...
        if (!parsed) {
          ...
          // try to sport a virus name once we know its not a scientific name
    ...
    postAssertParsing(pn, scientificName, name, passNo);
    ...
    if (pn.type == null) {
      ...
      if (!m.find()) {
        pn.type = NameType.doubtful;
      } else {
        // a wellformed, code compliant name?
        if (scientificName.equals(pn.canonicalNameWithAuthorship())) {
          pn.type = NameType.wellformed;
        } else {
          pn.type = NameType.sciname;
        }
      }
    }
  ...
  private void postAssertParsing(ParsedName<?> pn, final String rawName, final String normedName, final int passNo)
    throws UnparsableException {
    // if we only match a monomial in the 3rd pass its suspicious
    if (pn.genusOrAbove != null && !pn.isBinomial()) {
  ...
  public String parseToCanonical(String scientificName) {
    ...
    try {
      ParsedName<Integer> pn = parse(scientificName);
      if (pn != null) {

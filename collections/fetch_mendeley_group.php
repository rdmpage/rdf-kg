...
// add group to CouchDB, and enqueue DOIs and PMIDs found in group

require(dirname(dirname(__FILE__)) . '/documentstore/couchsimple.php');
require(dirname(dirname(__FILE__)) . '/queue/queue.php');


// go to https://mendeley-show-me-access-tokens.herokuapp.com to get

$token = 'MSwxNDc2MDE0NTQ5MjMwLDU3MjQsMTAyOCxhbGwsLCwsNDA3MGQxMjItYjNhZC0zZWRlLTk3ZWEtZDIzMzc3MGMzNjZhLE5wX0YxSms4VU01T0cyOHJaZkM0UHJ4SERiZw'; 
...
while (!$done)
{
	...
	else
	{
		...
		if ($http_code == 200)
		{
			...
			// Extract identifiers and add to queue
			foreach ($doc->message as $item)
			{
				if (isset($item->identifiers))
				{
					foreach ($item->identifiers as $k => $v)
					{
						switch ($k)
						{
							case 'doi':
								...
								$doi = preg_replace('/<\/p>/', '', $doi);
								if (preg_match('/^10\./', $doi))
								{
									enqueue('http://dx.doi.org/' . $doi); 
								}
								break;
								
							case 'pmid':
								$pmid = $v;
								enqueue('http://www.ncbi.nlm.nih.gov/pubmed/' . $pmid); 
								break;
